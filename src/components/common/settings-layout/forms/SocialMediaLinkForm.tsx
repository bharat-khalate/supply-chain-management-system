import { SocialMediaLinkFormFieldConstants } from "@/utils/constants";
import { shouldShowError } from "@/utils/validations";
import { SocialMediaLinkFormSchema } from "@/validations/settings/socialMediaLinkForm.validation";
import { Button, FieldError, Input, Label, TextField } from "@heroui/react";
import { useFormik } from "formik";
import AppDotLoader from "../../NavigationDotloader";

export interface ISocialMediaLinkFormFields {
    twitterLink: string;
    instagramLink: string;
    facebookLink: string;
}
export default function SocialMediaLinkForm(): React.ReactNode {
    const initialValues: ISocialMediaLinkFormFields = {
        twitterLink: "",
        instagramLink: "",
        facebookLink: ""
    }
    ...
    return (
        <form className="flex flex-col gap-y-6">
            <TextField isRequired isInvalid={shouldShowError(formik, SocialMediaLinkFormFieldConstants.facebookLink.key)}>
                <Label className={labelInputClass}>{SocialMediaLinkFormFieldConstants.facebookLink.label}</Label>
                <Input
                    className={inputClassName}
                    onBlur={formik.handleBlur}
                    onChange={formik.handleChange}
                    value={formik.values[SocialMediaLinkFormFieldConstants.facebookLink.key]}
                ...
                />
                {shouldShowError(formik, SocialMediaLinkFormFieldConstants.facebookLink.key) && (
                    <FieldError className={inputErrorMessageClass}>
                        {formik.errors[SocialMediaLinkFormFieldConstants.facebookLink.key]}
                    </FieldError>
                )}
            </TextField>
            <TextField isRequired isInvalid={shouldShowError(formik, SocialMediaLinkFormFieldConstants.instagramLink.key)}>
                <Label className={labelInputClass}>{SocialMediaLinkFormFieldConstants.instagramLink.label}</Label>
                <Input
                    className={inputClassName}
                    onBlur={formik.handleBlur}
                    onChange={formik.handleChange}
                    value={formik.values[SocialMediaLinkFormFieldConstants.instagramLink.key]}
                ...
                />
                {shouldShowError(formik, SocialMediaLinkFormFieldConstants.instagramLink.key) && (
                    <FieldError className={inputErrorMessageClass}>
                        {formik.errors[SocialMediaLinkFormFieldConstants.instagramLink.key]}
                    </FieldError>
                )}
            </TextField>
            <TextField isRequired isInvalid={shouldShowError(formik, SocialMediaLinkFormFieldConstants.twitterLink.key)}>
                <Label className={labelInputClass}>{SocialMediaLinkFormFieldConstants.twitterLink.label}</Label>
                <Input
                    className={inputClassName}
                    onBlur={formik.handleBlur}
                    onChange={formik.handleChange}
                    value={formik.values[SocialMediaLinkFormFieldConstants.twitterLink.key]}
                    name={SocialMediaLinkFormFieldConstants.twitterLink.key}
                    placeholder="Enter the Instagram profile url"
                    aria-label={SocialMediaLinkFormFieldConstants.twitterLink.label}
                />
                {shouldShowError(formik, SocialMediaLinkFormFieldConstants.twitterLink.key) && (
                    <FieldError className={inputErrorMessageClass}>
